  ...
  return (
    <MotionDiv>
      ...
      <main className="relative z-10 flex flex-col-reverse p-6 md:px-24  lg:pt-12 lg:min-h-[80vh] lg:grid lg:grid-cols-2">
        <div className="flex flex-col-reverse md:flex-col relative z-10 lg:flex-col lg:justify-between">
          ...
          <div className="text-secondary text-center lg:text-left">
            ...
            <h3 className="uppercase text-white mb-4 text-[24px] lg:text-[56px] ">
              {" "}
              {member.name}{" "}
            </h3>
            <p className="leading-relaxed  px-20 lg:px-0 lg:text-[18px] ">
              {" "}
              {member.description}{" "}
            </p>
          </div>
          <div className="flex gap-x-6 my-8 justify-center lg:justify-start">
            {members.map(({ name }, index) => (
              <div
                ...
                key={index}
                className={`w-[10px] aspect-square rounded-full transition cursor-pointer opacity-40 bg-white [&.active]:opacity-100 [&:not(.active)]:hover:opacity-75 ${
